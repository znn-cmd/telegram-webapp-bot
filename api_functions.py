...
def get_locations() -> Dict[str, Any]:
    ...
    # Получаем данные из таблицы locations
    url = f"{SUPABASE_URL}/rest/v1/locations?select=*"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for loc in locations:
                ...
                if country and country not in structured_locations['countries']:
                    structured_locations['countries'].append(country)
                
                # Добавляем город в соответствующую страну
                if country and city:
                    if country not in structured_locations['cities']:
                        structured_locations['cities'][country] = []
                    if city not in structured_locations['cities'][country]:
                        structured_locations['cities'][country].append(city)
                
                # Добавляем регион в соответствующий город
                if city and region:
                    city_key = f"{country}:{city}"
                    if city_key not in structured_locations['regions']:
                        structured_locations['regions'][city_key] = []
                    if region not in structured_locations['regions'][city_key]:
                        structured_locations['regions'][city_key].append(region)
                
                # Добавляем район в соответствующий регион
                if region and district:
                    region_key = f"{country}:{city}:{region}"
                    if region_key not in structured_locations['districts']:
                        structured_locations['districts'][region_key] = []
                    if district not in structured_locations['districts'][region_key]:
                        structured_locations['districts'][region_key].append(district)
            
            return {
                'success': True,
                'data': structured_locations
            }
        else:
            return {
                'success': False,

...
2. **matplotlib.category Info Messages**:
   - Конвертация строковых годов в целые числа: `years = [int(year) for year in years]`
...
### Исправление предупреждений о шрифтах fpdf2
**Проблема**: `DeprecationWarning: "uni" parameter is deprecated since v2.5.1, unused and will soon be removed`

**Решение**:
1. **Удаление устаревшего параметра**: Убран параметр `uni=True` из всех вызовов `pdf.add_font()`
2. **Применено к обеим функциям**:
   - `api_generate_pdf_report()` - основная генерация PDF
   - `api_send_saved_report_pdf()` - сохраненные отчеты
3. **Тестирование**: Создан тестовый скрипт для проверки загрузки шрифтов без `uni=True`

**Результат**: ✅ Предупреждения о шрифтах fpdf2 устранены, приложение больше не закрывается при генерации PDF

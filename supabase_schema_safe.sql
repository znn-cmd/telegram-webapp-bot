-- Безопасное создание таблиц для бота недвижимости в Supabase
...
-- Создание индексов для улучшения производительности (безопасно)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_short_term_rentals_district') THEN
        CREATE INDEX idx_short_term_rentals_district ON short_term_rentals(district);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_short_term_rentals_price') THEN
        CREATE INDEX idx_short_term_rentals_price ON short_term_rentals(price_per_night);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_short_term_rentals_bedrooms') THEN
        CREATE INDEX idx_short_term_rentals_bedrooms ON short_term_rentals(bedrooms);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_short_term_rentals_location') THEN
        CREATE INDEX idx_short_term_rentals_location ON short_term_rentals(latitude, longitude);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_long_term_rentals_district') THEN
        CREATE INDEX idx_long_term_rentals_district ON long_term_rentals(district);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_long_term_rentals_price') THEN
        CREATE INDEX idx_long_term_rentals_price ON long_term_rentals(monthly_rent);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_long_term_rentals_bedrooms') THEN
        CREATE INDEX idx_long_term_rentals_bedrooms ON long_term_rentals(bedrooms);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_long_term_rentals_location') THEN
        CREATE INDEX idx_long_term_rentals_location ON long_term_rentals(latitude, longitude);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_property_sales_district') THEN
        CREATE INDEX idx_property_sales_district ON property_sales(district);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_property_sales_price') THEN
        CREATE INDEX idx_property_sales_price ON property_sales(asking_price);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_property_sales_bedrooms') THEN
        CREATE INDEX idx_property_sales_bedrooms ON property_sales(bedrooms);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_property_sales_location') THEN
        CREATE INDEX idx_property_sales_location ON property_sales(latitude, longitude);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_historical_prices_property') THEN
        CREATE INDEX idx_historical_prices_property ON historical_prices(property_id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_historical_prices_date') THEN
        CREATE INDEX idx_historical_prices_date ON historical_prices(date);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_historical_prices_type') THEN
        CREATE INDEX idx_historical_prices_type ON historical_prices(price_type);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_market_statistics_district') THEN
        CREATE INDEX idx_market_statistics_district ON market_statistics(district);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_market_statistics_date') THEN
        CREATE INDEX idx_market_statistics_date ON market_statistics(date);
    END IF;
END $$;
...
-- Создание политик для публичного доступа (только чтение) - безопасно
DO $$
BEGIN
    -- Политики для short_term_rentals
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'short_term_rentals' AND policyname = 'Allow public read access') THEN
        CREATE POLICY "Allow public read access" ON short_term_rentals FOR SELECT USING (true);
    END IF;
    
    -- Политики для long_term_rentals
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'long_term_rentals' AND policyname = 'Allow public read access') THEN
        CREATE POLICY "Allow public read access" ON long_term_rentals FOR SELECT USING (true);
    END IF;
    
    -- Политики для property_sales
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'property_sales' AND policyname = 'Allow public read access') THEN
        CREATE POLICY "Allow public read access" ON property_sales FOR SELECT USING (true);
    END IF;
    
    -- Политики для historical_prices
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'historical_prices' AND policyname = 'Allow public read access') THEN
        CREATE POLICY "Allow public read access" ON historical_prices FOR SELECT USING (true);
    END IF;
    
    -- Политики для market_statistics
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'market_statistics' AND policyname = 'Allow public read access') THEN
...
-- Создание триггеров для автоматического обновления updated_at (безопасно)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_short_term_rentals_updated_at') THEN
        CREATE TRIGGER update_short_term_rentals_updated_at 
            BEFORE UPDATE ON short_term_rentals 
            FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_long_term_rentals_updated_at') THEN
        CREATE TRIGGER update_long_term_rentals_updated_at 
            BEFORE UPDATE ON long_term_rentals 
            FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_property_sales_updated_at') THEN
